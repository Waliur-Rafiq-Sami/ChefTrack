...
const MyProfile = () => {
  ...
  // Use useEffect to update formData when the user object is available
  useEffect(() => {
    if (user) {
      setFormData({
        ...
        address: user.address || "", // Initialize address from user object
      });
    }
  }, [user]);
  ...
  const handleSave = () => {
    ...
    // to update the user's profile with the new formData.
    // For this example, we'll just log the data and toggle editing off.
  ...
  const handleCancel = () => {
    // Reset the form data to the original user values
    setFormData({
      displayName: user.displayName || "",
      email: user.email || "",
      phone: user.phone || "",
      ...
      <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-2xl md:p-8 p-3 max-w-3xl w-full border border-white/30 pb-7">
        ...
        <div className="flex flex-col items-center">
          <img
            src={
              isEditing
                ? formData.photoURL || "https://i.ibb.co/4pDNDk1/avatar.png"
